...
def main():
    ...
    GPIO.add_event_detect(PIN_BUTTON, GPIO.FALLING, callback=button_pressed, bouncetime=200)
    ...
    while not exit_flag.wait(timeout=0.25):
        ...
        with event_lock:
            ...
            if new_volume_effects != volume_effects:
                ...
                print "Effects volume changed to %s" % volume_music
        ...
        if not pygame.mixer.music.get_busy() or move_to_next_song:
            next_song = player.next_song(is_a)
            if next_song:
